        ...
        case ESP_GAP_BLE_SCAN_RESULT_EVT: {
            ...
            switch (scan_result->scan_rst.search_evt) {
                case ESP_GAP_SEARCH_INQ_RES_EVT: {
                    //print_hex("dataSalt ", scan_result->scan_rst.ble_adv, scan_result->scan_rst.adv_data_len);   
...
static void buff_callback(void* arg){
    ...
    if (item != NULL && buf_handle != NULL) {
        ...
        //print_hex("dataSalt ", *item, sizeof data);  
        
        awsiot_send_data(item, item_size);

        //print_hex2("dataSalt ", cPayload, sizeof cPayload);  
